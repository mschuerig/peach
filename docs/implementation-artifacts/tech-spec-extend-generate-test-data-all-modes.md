...
- [x] Task 1: Add interval constants and helpers
...
- [x] Task 2: Fix and generalize pitch comparison row generator
  - File: `bin/generate-test-data.py`
...
- [x] Task 3: Add pitch matching row generator
  - File: `bin/generate-test-data.py`
...
- [x] Task 4: Refactor record generation to support modes
  - File: `bin/generate-test-data.py`
...
- [x] Task 5: Add argparse CLI and update main
  - File: `bin/generate-test-data.py`
...
- Findings: 8 total, 5 fixed, 3 skipped (noise/undecided)
- Resolution approach: auto-fix
- F1: centOffset now includes negative values (real app range)
- F2: initialCentOffset clamped to -20..20 (matches app's actual range)
- F3: Day bucket timestamps anchored to calendar days, no bleed
- F4: Added `else: raise ValueError` in `make_row` for unknown modes
- F5: Removed `max(1, ...)` from bucket counts; small --count now exact
