...
struct ChartImageRendererTests {
    ...
    func exportFileNameFormat() async {
        ...
        let fileName = ChartImageRenderer.exportFileName(for: date, mode: .unisonPitchComparison)
        #expect(fileName == "peach-pitch-comparison-2026-03-15-1432.png")
    }

    @Test("export filename for interval comparison mode")
    func exportFileNameIntervalComparison() async {
        let date = makeDate(year: 2026, month: 3, day: 15, hour: 14, minute: 32)
        let fileName = ChartImageRenderer.exportFileName(for: date, mode: .intervalPitchComparison)
        #expect(fileName == "peach-interval-comparison-2026-03-15-1432.png")
    }

    @Test("export filename for pitch matching mode")
    func exportFileNamePitchMatching() async {
        let date = makeDate(year: 2026, month: 3, day: 15, hour: 14, minute: 32)
        let fileName = ChartImageRenderer.exportFileName(for: date, mode: .unisonMatching)
    ...
    func exportFileNameIntervalMatching() async {
        let date = makeDate(year: 2026, month: 3, day: 15, hour: 14, minute: 32)
        let fileName = ChartImageRenderer.exportFileName(for: date, mode: .intervalMatching)
    ...
    func exportFileNameLocaleIndependent() async {
        ...
        let fileName = ChartImageRenderer.exportFileName(for: date, mode: .unisonPitchComparison)
        #expect(fileName == "peach-pitch-comparison-2026-01-05-0903.png")
    }
