...
struct KazezNoteStrategyTests {
    ...
    func firstComparisonCustomMax() {
        let strategy = KazezNoteStrategy()
        let settings = TrainingSettings(maxCentDifference: 50.0)
    ...
    func floorClamping() {
        let strategy = KazezNoteStrategy()
        let settings = TrainingSettings(minCentDifference: 5.0)
    ...
    func convergenceTest() {
        ...
        for _ in 0..<10 {
            let completed = CompletedComparison(
                ...
                userAnsweredHigher: comparison.isSecondNoteHigher // correct
            )
    ...
    private func nextAfterCorrect(p: Double) -> Double {
        let strategy = KazezNoteStrategy()
        let settings = TrainingSettings(minCentDifference: 0.1)
    ...
    private func nextAfterIncorrect(p: Double) -> Double {
        let strategy = KazezNoteStrategy()
        let settings = TrainingSettings(maxCentDifference: 200.0)
