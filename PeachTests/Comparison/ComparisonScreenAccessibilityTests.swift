...
/// Tests for ComparisonScreen accessibility features (Story 7.2)
@Suite("ComparisonScreen Accessibility Tests")
struct ComparisonScreenAccessibilityTests {
    ...
    func feedbackAnimationNilWhenReduceMotion() async throws {
        let animation = ComparisonScreen.feedbackAnimation(reduceMotion: true)
    ...
    func feedbackAnimationPresentWhenNoReduceMotion() async throws {
        let animation = ComparisonScreen.feedbackAnimation(reduceMotion: false)
        #expect(animation == .easeInOut(duration: 0.2))
    }

    // MARK: - ComparisonFeedbackIndicator Accessibility Labels (AC: #2)

    @Test("ComparisonFeedbackIndicator correct state returns non-empty label")
    func feedbackIndicatorCorrectLabel() async throws {
        let label = ComparisonFeedbackIndicator.accessibilityLabel(isCorrect: true)
        #expect(!label.isEmpty)
    }

    @Test("ComparisonFeedbackIndicator incorrect state returns non-empty label")
    func feedbackIndicatorIncorrectLabel() async throws {
        let label = ComparisonFeedbackIndicator.accessibilityLabel(isCorrect: false)
        #expect(!label.isEmpty)
    }

    @Test("ComparisonFeedbackIndicator correct and incorrect labels are distinct")
    func feedbackIndicatorLabelsDistinct() async throws {
        let correctLabel = ComparisonFeedbackIndicator.accessibilityLabel(isCorrect: true)
        let incorrectLabel = ComparisonFeedbackIndicator.accessibilityLabel(isCorrect: false)
    ...
    func higherLowerLocalizationKeysDistinct() async throws {
        // Note: ComparisonScreen applies these as .accessibilityLabel("Higher") / .accessibilityLabel("Lower")
